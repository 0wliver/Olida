...
y_pred = lgbm.predict(X_test) # 预测用于实际模型，以及进行更多的分析，如auc
...
# 误差下降图
epochs = len(results['valid_0']['rmse'])
x_axis = range(0, epochs)
# plot log loss
fig, ax = plt.subplots()
ax.plot(x_axis, results['valid_0']['rmse'], label='Train')
ax.plot(x_axis, results['valid_1']['rmse'], label='Test')
ax.legend()
plt.ylabel('RMSE')
plt.title('LightGBM RMSE on Dataset-Boston')
plt.show()
